...
class MathMillionaireGame {
    ...
    startGame() {
        ...
        // Store session ID in localStorage for audience page access
    ...
    loadQuestion() {
        ...
        if (this.sessionId) {
            ...
            localStorage.setItem(`gameSession_${this.sessionId}_questionData`, JSON.stringify({
                question: question.question,
                options: question.options,
                questionNumber: this.currentQuestion + 1,
                correct: question.correct, // Store correct answer for score calculation
                assignedClass: assignedClass // Store assigned class
            }));
            // Store question data with index for score calculation
            localStorage.setItem(`gameSession_${this.sessionId}_questionData_${this.currentQuestion}`, JSON.stringify({
                question: question.question,
                options: question.options,
            ...
            // Clear previous audience votes for this question
    ...
    checkAnswer(selectedIndex, question) {
        ...
        if (isCorrect) {
            ...
            if (assignedClass && this.sessionId) {
                this.awardClassPoints(assignedClass, 100000);
            }
    ...
    awardClassPoints(className, points) {
        if (!this.sessionId) return;
        
        // Get current class scores from session
        const scoresKey = `gameSession_${this.sessionId}_classScores`;
        let classScores = {};
    ...
    showStatistics() {
        ...
        const classScores = this.calculateClassScores(currentSession);
    ...
    calculateClassScores(sessionId) {
        if (!sessionId) return {};
